...
%     solves the noise constrained sparse nonnegat
%-------------------------------------------------------------------------%
...
switch lower(options.method)
    case 'foopsi'  %% use FOOPSI
        % 19Mar2020, Edited KS, sometimes options.pars returns multiple arguments which breaks the code (not sure why). To
        % account for this, I just take the first value (which is fine, because usually it's only 1 value anyway...)
        if strcmpi(options.type, 'ar1')  % AR 1
            [c, s, options.b, options.g] = foopsi_oasisAR1(y-options.b, options.pars(1), options.lambda, ...
