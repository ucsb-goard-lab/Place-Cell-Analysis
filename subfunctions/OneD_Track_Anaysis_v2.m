...
%-------------------------------------------------------------------------%
%ONED_TRACK_ANAYSIS_V2 Bins the linear track data by angle and finds laps.
%
%   More specifically, the function bins the linear track data by angle
%   (the size of the bins being given by degree_size). This can be used for any of the HPC data
%   where the mouse ran on the track. The first thing this function does is find the laps.
%   It then disregards any activity that doesn't fall in one of the laps.
%
%   Inputs:
%   'data' is a structure containing the suite2p activity data, with fields 'DFF' and 'DFF_transients'.
%   'floating' is a structure containing the behavior data, with a field 'phi' that is a vector of the mouse's angular position at each time point.
%   'degree_size' is a scalar that specifies the size of the bins for binning the angles.
%   'trialID' is a string identifier for the trial.
%
%   Outputs:
%   'activity_binned_1D_sde_smoothed' is an array containing the smoothed standard error of the mean (SEM) of the binned activity data.
%   'activity_binned_1D_smoothed' is an array containing the smoothed binned activity data.
%   'binned_angles' is a vector of the binned angular positions of the mouse.
%   'consistency' is a measure of the consistency of the activity patterns across laps.
%   'lap_times' is a matrix where each row contains the start and end times of a lap.
